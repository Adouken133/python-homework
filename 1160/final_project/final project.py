...
# Drop useless Column
vgsale.drop(['Name', 'Genre', 'Platform', 'Publisher'], axis=1, inplace=True)
...
plt.scatter(vgsale['Genre_new'], vgsale['Global_Sales'], alpha=0.7, label='Global_Sales')
plt.scatter(vgsale['Genre_new'], vgsale['JP_Sales'], alpha=0.7, label='JP_Sales')
plt.scatter(vgsale['Genre_new'], vgsale['NA_Sales'], alpha=0.7, label='NA_Sales')
plt.xlabel('Genre_new')
plt.ylabel('Global_Sales / JP_Sales / Global_Sales')
...
plt.figure(figsize=(10,10))
plt.scatter(vgsale['Genre_new'], vgsale['Global_Sales'], s=(vgsale['JP_Sales']) ** 2.5,
             label=f'Genre_new to Global_Sales', color='orange', marker='o', edgecolors='w', alpha=0.7)

plt.xlabel(f'Genre_new')
plt.ylabel(f'Global_Sales')
plt.title(f'Alcohol, Malic Acid and Color Intensity (Size)')
